...
from always_on_finance_briefing_agent.scheduler_api import app, run_scheduled_scout


def test_scheduled_scout_defaults_to_dry_run_with_article():
    result = run_scheduled_scout({"top_n": 2})

    assert result["dry_run"] is True
    assert result["delivery"]["status"] == "dry_run"
    assert len(result["brief"]["filings"]) == 2
    assert "markdown" in result["brief"]["article"]


def test_scheduled_scout_can_skip_article():
    result = run_scheduled_scout({"top_n": 1, "include_article": False})

    assert "article" not in result["brief"]
...
def test_trigger_endpoint_accepts_scheduler_payload():
    ...
    assert len(payload["brief"]["filings"]) == 1
...
def test_pubsub_endpoint_decodes_scheduler_message():
    ...
    response = client.post("/finance-brief/pubsub", json={"message": {"data": message}})
...
def test_trigger_endpoint_tolerates_empty_body():
    ...
    response = client.post("/finance-brief/trigger")
...
def test_explicit_gmail_delivery_reports_missing_config(monkeypatch):
    monkeypatch.setenv("FINBRIEF_DELIVERY", "gmail")
    for name in [
        "FINBRIEF_EMAIL_TO",
        "FINBRIEF_EMAIL_FROM",
        "FINBRIEF_GMAIL_CLIENT_ID",
